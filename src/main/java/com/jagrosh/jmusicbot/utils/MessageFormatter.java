...
public class MessageFormatter {
    ...
    public static MessageCreateData buildNowPlayingMessage(Bot bot, NowPlayingInfo info) {
        // no modo fase o áudio não vem de um AudioTrack: a tela é montada a partir das fases
        if (info.segmentPlayer != null)
            return PhaseMessageFormatter.buildPhaseNowPlaying(bot, info);
    ...
    private static void applyNowPlayingButtons(Bot bot, MessageCreateBuilder mb, NowPlayingInfo info, RepeatMode repeatMode) {
        ...
        mb.setComponents(
                ...
                ActionRow.of(phaseRowButtons(bot, info))
        );
    }

    /**
     * A linha do modo fase. O botão de trocar de modo só aparece quando a faixa tocando tem
     * fases cadastradas — oferecer a troca numa faixa sem segmentação só levaria a um erro.
     */
    private static java.util.List<Button> phaseRowButtons(Bot bot, NowPlayingInfo info) {
        java.util.List<Button> buttons = new java.util.ArrayList<>();
        buttons.add(PhaseMessageFormatter.openPanelButton());
        if (info.track != null && bot.getPhaseService().findMatchingPhases(info.track) != null)
            buttons.add(PhaseMessageFormatter.switchToPhasesButton());
        return buttons;
    }

    static String nowPlayingButtonId(String action) {
