...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler 
{
    ...
    /** Quando não é nulo, os frames vêm daqui em vez do lavaplayer (modo fase). */
    private volatile SegmentPlayer segmentPlayer;
    private ByteBuffer pendingSegmentFrame;
    private volatile boolean lastFrameIsPcm;
    /** A mensagem do now-playing do modo fase, para editar em vez de spammar uma nova a cada
     *  troca de fase automática (a troca acontece na thread de áudio, sem uma interação do
     *  Discord à mão para editar diretamente). */
    private volatile net.dv8tion.jda.api.entities.Message segmentNowPlayingMessage;
    ...
    public boolean isMusicPlaying(JDA jda)
    {
        ...
        // no modo fase o áudio não vem do lavaplayer, mas o bot está tocando do mesmo jeito
        var isAudioPlaying = audioPlayer.getPlayingTrack() != null || segmentPlayer != null;
    ...
    {
        SegmentPlayer segments = segmentPlayer;
        return new NowPlayingInfo(
            audioPlayer.getPlayingTrack(),
            jda.getGuildById(guildId),
            segments != null ? segments.isPaused() : audioPlayer.isPaused(),
    ...
    /**
     * Assume (ou libera, com null) a saída de áudio do guild para o modo fase.
     */
    public void setSegmentPlayer(SegmentPlayer player)
    {
        this.segmentPlayer = player;
    }

    public SegmentPlayer getSegmentPlayer()
    {
        return segmentPlayer;
    }

    public void setSegmentNowPlayingMessage(net.dv8tion.jda.api.entities.Message message)
    {
        this.segmentNowPlayingMessage = message;
    }

    public net.dv8tion.jda.api.entities.Message getSegmentNowPlayingMessage()
    {
        return segmentNowPlayingMessage;
    }
    ...
    public boolean canProvide()
    {
        ...
        if (segments != null)
        {
            if (!segments.canProvide())
            {
                lastFrameIsPcm = false;
                return false;
            }
            pendingSegmentFrame = segments.provide20MsAudio();
            lastFrameIsPcm = pendingSegmentFrame != null;
            return lastFrameIsPcm;
        }
    ...
    {
        if (pendingSegmentFrame != null)
        {
            ByteBuffer frame = pendingSegmentFrame;
            pendingSegmentFrame = null;
            return frame;
        }
    ...
    {
        // consultado pelo JDA depois de provide20MsAudio, por pacote: responde sobre o frame
        // que acabou de sair, não sobre o estado atual (que pode ter mudado no meio)
        return !lastFrameIsPcm;
    }
}
